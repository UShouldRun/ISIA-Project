...
Mixin class to add visualization capabilities to your SPADE agents
Place this file in your src/agents/ directory
"""
...
class VisualizationBehaviour(CyclicBehaviour):
    """
    Periodic behaviour that sends agent updates to visualization.
    Add this behaviour to agents that need real-time visualization.
    """
    
    def __init__(self, update_interval=0.1):
        """
        Args:
            update_interval: Time in seconds between updates (default 0.1 = 10 FPS)
        """
        super().__init__()
    ...
    async def run(self):
        ...
        if hasattr(self.agent, 'viz_send_update'):
